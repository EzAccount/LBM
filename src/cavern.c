    ...
    for (int t = 0; t < time; t++){
        ...
        for (int i=1; i<x_size-1; ++i){
            ...
            *(ftemp_point + 9 * (x_size + 1) + 5) = *(f_point+5);
            ...
            vel_point = vel + i * 2;
            // transfer
            *(gtemp_point + 9 * x_size + 2) = *(geq_point + 2);
            *(gtemp_point + 9 * (x_size - 1) + 6) = *(geq_point + 6);
            *(gtemp_point + 9 * (x_size + 1) + 5) = *(geq_point + 5);
        }
        // Corners:
        // left-bot:
        ftemp_point = f_temp;
        ...
        *(gtemp_point + 9 * (x_size - 1) + 6) = *(geq_point+6);
        ...
        for (int i=1; i<x_size-1; ++i){
            ...
            *(ftemp_point - 9 * (x_size - 1) + 8) = *(f_point+8);
            ...
            geq_point = g_eq +(y_size-1)*x_size*9+ i*9;
            // transfer
            *(gtemp_point - 9 * x_size + 4) =*(geq_point+4);
            *(gtemp_point - 9 * (x_size + 1) + 7) =*(geq_point+7);
            *(gtemp_point - 9 * (x_size - 1) + 8) =*(geq_point+8);
        }
        ...
        *(ftemp_point - 9 * (x_size - 1) + 8) = *(f_point+8);
        ...
        geq_point = g_eq + (y_size-1)*x_size*9;
        *(gtemp_point - 9 * (x_size - 1) + 8) = *(geq_point+8);
        //right-top:
        ...
        *(ftemp_point - 9 * (x_size + 1) + 7) = *(f_point+7);
        ...
        geq_point = g_eq + (y_size-1)*x_size*9 + x_size*9 - 9;
        *(gtemp_point - 9 * (x_size + 1) + 7) = *(geq_point+7);
        ...
        for (int j = 1; j < y_size-1; ++j){
            ...
            *(ftemp_point + 9 * (x_size + 1) + 5) = *(f_point+5);
            *(ftemp_point - 9 * (x_size - 1) + 8) = *(f_point+8);
            gtemp_point = g_temp + j*x_size*9;
            geq_point = g_eq + j*x_size*9;
            *(gtemp_point + 9 + 1) = *(geq_point + 1);
            *(gtemp_point + 9 * (x_size + 1) + 5) = *(geq_point+5);
            *(gtemp_point - 9 * (x_size - 1) + 8) =*(geq_point+8);
            //
            ...
            *(ftemp_point - 9 * (x_size + 1) + 7) = *(f_point + 7);
            ...
            geq_point = g_eq + j*x_size*9 + 9 *(x_size-1);
            *(gtemp_point - 9 + 3) =*(geq_point+3);
            *(gtemp_point + 9 * (x_size - 1) + 6) =*(geq_point + 6);
            *(gtemp_point - 9 * (x_size + 1) + 7) =*(geq_point + 7);
        }
        ...
        *(f_point+7) = *(ftemp_point+5);
        ...
        for (int i=1; i<x_size-1; ++i){
            ...
            g_point = g + i*9;
            feq_point = f_eq + i*9;
            bal = (*(ftemp_point+4) + *(ftemp_point+7) + *(ftemp_point+8))/(*(feq_point+2)+*(feq_point+5)+*(feq_point+6));
