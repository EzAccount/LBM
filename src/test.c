...
double mu_powerlow(double temperature){              // \mu = A \cdot T ^ {omega}
  double omega = 0.8;
  double A = 1.7;       // A = mu_rest \cdot T_rest ^{-omega}
...
int main()
{
  ...
  ////
  int random;
  int time = 2002; // steps in time
  FILE * debug = fopen("data/lingrad.debug.txt", "w");
  FILE * veldat = fopen("data/lingrad.vel.dat", "w");
  FILE * rho = fopen("data/lingrad.rho.dat", "w");
  FILE * Tdat = fopen("data/lingrad.T.dat", "w");
  FILE * veldat2 = fopen("data/lingrad.vel2.dat", "w");
  FILE * sech = fopen("data/lingrad.sech.dat", "w");
  FILE * rashod = fopen("data/lingrad.rashod.dat", "w");
  ...
  for (int i = 0; i < y_size; ++i){
    ...
    for (int j = 0; j < x_size; ++j){
        ...
        rho_point[j][i] = ((j+1)*(P2-P1)/x_size + P1)/ Tw;//
        P[j][i] = ((j+1)*(P2-P1)/x_size + P1);
        T[j][i] = Tw;
  ...
  for (int t = 0; t < time; ++t){
    ...
    for (int i=1; i<x_size-1; ++i){
      ...
      *(f_point + 8) = *(ftemp_point+5);
      *(f_point + 7) = *(ftemp_point+6);
      *(f_point + 4) = *(ftemp_point+2);
    ...
    *(ftemp_point + 9 * (x_size - 1) + 7) = *(f_point+7);
    ...
    for (int i=1; i<x_size-1; ++i){
      ...
      *(f_point + 5) = *(ftemp_point+8);
      *(f_point + 6) = *(ftemp_point+7);
      *(f_point + 2) = *(ftemp_point+4);
    }
    //Corners:
    //left-bot:
  ...
  //   fprintf(sech, "%d %f \n", i,  rho_point[x_size/2][i]);
  for (int i=1; i<y_size-1; i++)
    fprintf(sech, "%d %f \n", i,  vel[(x_size/2)*2*i + 2*(y_size/2)]);
  double result1 = 0;
  for (int j=1; j<x_size-1; j++){
  for (int i=1; i<y_size-1; i++){
    result1 += rho_point[j][i] * vel[j*2+i*2*x_size];
    }
    fprintf(rashod, "%d %f\n", j, result1);
  }
